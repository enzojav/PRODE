...
// ─── BRACKET: PARTIDOS R16/QF/SF/FINAL ───────────────────────
// Devuelve todos los partidos de fase eliminatoria con la predicción del usuario
router.get('/bracket/matches', requireAuth, async (req, res) => {
  try {
    const { rows: matches } = await db.query(
      "SELECT * FROM prode_matches WHERE phase IN ('R16','QF','SF','F') ORDER BY id"
    );
    // Predicciones de este usuario para estos partidos
    const ids = matches.map(m => m.id);
    let preds = [];
    if (ids.length > 0) {
      const { rows } = await db.query(
        'SELECT * FROM prode_predictions WHERE username=$1 AND match_id = ANY($2)',
        [req.user.username, ids]
      );
      preds = rows;
    }
    const result = matches.map(m => {
      const p = preds.find(p => p.match_id === m.id) || null;
      return { ...m, my_prediction: p ? p.result : null };
    });
    res.json(result);
  } catch (e) {
    console.error(e);
...
// Votar ganador de un partido eliminatorio (solo '1' o '2')
router.post('/bracket/vote', requireAuth, async (req, res) => {
  const { match_id, result } = req.body;
  if (!match_id || !['1','2'].includes(result))
    return res.status(400).json({ error: 'Datos inválidos. result debe ser "1" o "2".' });

  try {
    const { rows: matchRows } = await db.query(
      "SELECT * FROM prode_matches WHERE id=$1 AND phase IN ('R16','QF','SF','F')",
      [match_id]
    );
    const match = matchRows[0];
    if (!match) return res.status(404).json({ error: 'Partido no encontrado.' });

    // Verificar lock (1 hora antes)
    try {
      const parts = (match.match_date || '').split(' ');
      const day   = parseInt(parts[1]);
      const month = MONTH_MAP[parts[2]];
      ...
      const lockTime  = new Date(matchTime.getTime() - 60 * 60 * 1000);
      if (new Date() >= lockTime)
        return res.status(403).json({ error: 'Predicción cerrada (1 hora antes del inicio).' });
    } catch {}
    ...
    if (exists.rows.length > 0) {
      const { rows } = await db.query(
        'UPDATE prode_predictions SET result=$1, home_score=NULL, away_score=NULL WHERE username=$2 AND match_id=$3 RETURNING *',
        [result, req.user.username, match_id]
      );
      saved = rows[0];
    } else {
      const { rows } = await db.query(
        'INSERT INTO prode_predictions (username, match_id, result, home_score, away_score) VALUES ($1,$2,$3,NULL,NULL) RETURNING *',
        [req.user.username, match_id, result]
      );
      saved = rows[0];
    }
    res.json(saved);
...
// Cargar resultado manual de un partido eliminatorio (admin)
router.put('/bracket/matches/:id/result', requireAuth, requireAdmin, async (req, res) => {
  const { home_score, away_score } = req.body;
  if (home_score === undefined || away_score === undefined)
    return res.status(400).json({ error: 'Faltan goles.' });
  ...
  try {
    const { rows } = await db.query(
      "UPDATE prode_matches SET home_score=$1, away_score=$2 WHERE id=$3 AND phase IN ('R16','QF','SF','F') RETURNING *",
      [hN, aN, req.params.id]
    );
    if (!rows[0]) return res.status(404).json({ error: 'Partido no encontrado.' });
    res.json(rows[0]);
...
// Borrar resultado de un partido eliminatorio (admin)
router.delete('/bracket/matches/:id/result', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { rows } = await db.query(
      "UPDATE prode_matches SET home_score=NULL, away_score=NULL WHERE id=$1 AND phase IN ('R16','QF','SF','F') RETURNING *",
      [req.params.id]
    );
    if (!rows[0]) return res.status(404).json({ error: 'Partido no encontrado.' });
    res.json(rows[0]);
