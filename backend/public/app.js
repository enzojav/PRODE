...
function renderR16Bracket() {
  ...
  function drawMatchCard(svg, m, x, y) {
    ...
    const pred    = r16Preds[m.id] || {};
    const locked  = flase //isMatchLocked(m);
    const mH      = m.home_score !== null && m.home_score !== undefined ? Number(m.home_score) : null;
    const mA      = m.away_score !== null && m.away_score !== undefined ? Number(m.away_score) : null;
    const played  = mH !== null && mA !== null;
    ...
    rows.forEach(({ flag, name, score, isW, isL, oy, side }) => {
      ...
      if (currentUser.role === 'admin') {
        const fo = el('foreignObject', { x: x + CW - 30, y: oy - 10, width: 28, height: 20 }, svg);
        const inp = document.createElement('input');
        inp.type = 'number'; inp.min = 0; inp.max = 20;
        inp.value = score !== null && score !== undefined ? score : '';
        inp.placeholder = '—';
        inp.style.cssText = 'width:28px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:4px;color:' + (isW ? AMBER : 'rgba(255,255,255,.5)') + ';font-size:11px;font-weight:800;text-align:center;padding:0;outline:none;';
        inp.addEventListener('change', e => setR16Result(m.id, side === '1' ? 'home' : 'away', e.target.value));
        fo.appendChild(inp);
